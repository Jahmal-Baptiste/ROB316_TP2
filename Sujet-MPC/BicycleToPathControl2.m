...
while size(list_points, 2) < window_size
  waypoint       = Path(:, goalWaypointId);
  err            = waypoint - xtemp;
  distToWaypoint = norm(err(1:2));
  if distToWaypoint < rho
    list_points    = [list_points, waypoint];
    goalWaypointId = goalWaypointId + 1;
    goalWaypointId = min(goalWaypointId, size(Path, 2)); %security
  else
    direction   = err/norm(err);
...
anticipation = min(window_size, size(list_points,2)) %which future points is the objective
